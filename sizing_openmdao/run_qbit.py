...
def solve_qbit(payload_kg: float = PAYLOAD_KG,
              range_m: float = RANGE_M,
              n_c: int = N_C,
              verbose: bool = True) -> QbitResult:
    prob = build_problem(payload_kg=payload_kg, range_m=range_m, n_c=n_c)

    if verbose:
        print('=' * 60)
        print('QBiT sizing model')
...
if __name__ == '__main__':
    result = solve_qbit(payload_kg=PAYLOAD_KG, range_m=RANGE_M, n_c=N_C, verbose=True)
