...
class AerodynamicTrimComp(om.ExplicitComponent):
    def setup(self):
        ...
        self.add_output('T_cruise', val=2.0, units='N')
        self.add_output('CL', val=0.3)
        self.add_output('CT', val=0.002)
        self.add_output('mu', val=0.04)
        self.add_output('lam', val=0.40)
        self.add_output('Vi', val=0.20, units='m/s')
        self.add_output('P0', val=15.0, units='W')
